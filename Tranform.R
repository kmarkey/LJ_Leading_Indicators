...
#read in raw, soon fetch with SQL
...
######### rework setting model year ##################
# highest possible model year = current year + 1
max_year <- as.numeric(year(Sys.Date())) + 1
# assuming no cars older than 100
# get last 2 digits
max_year <- as.numeric(substr(max_year, 3, 4))
