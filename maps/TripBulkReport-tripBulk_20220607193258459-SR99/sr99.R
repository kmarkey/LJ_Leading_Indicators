...
tripheads <- read_delim("TripBulkReport-tripBulk_20220607193258459-SR99/schema/TripBulkReportTripsHeaders.csv") |>
  names()

trips <- read_csv("TripBulkReport-tripBulk_20220607193258459-SR99/data/trips.csv", col_names = tripheads, trim_ws = TRUE)
...
start_income %>%
  ...
  labs(title = "SR99 Median Household Income", subtitle = "sample start",
...
end_income %>%
  uncount(endpoints) %>%
  ggplot() + geom_histogram(aes(x = mhi), color = "black", fill = "lightblue") + 
  labs(title = "SR99 Median Household Income", subtitle = "sample end",
       x = "Median Household Income", y = "Count") +
  theme_classic()
